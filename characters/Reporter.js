...
/**
 * Reporter — 外星人记者（扛摄像机版）
 * 绿色小外星人，三只眼睛，扛着摄像机采访选手
 * 身材矮小 (~1.2m)，圆滚滚的身体，短手短脚
 */
export class Reporter extends CharacterBase {
  constructor() {
    super('Reporter');
  ...
  build() {
    ...
    // Materials
    const skinMat = new THREE.MeshToonMaterial({ color: 0x7cb342, gradientMap: toonGradient });
    const skinDarkMat = new THREE.MeshToonMaterial({ color: 0x558b2f, gradientMap: toonGradient });
    ...
    const suitMat = new THREE.MeshToonMaterial({ color: 0x1a237e, gradientMap: toonGradient });
    const suitLightMat = new THREE.MeshToonMaterial({ color: 0x3949ab, gradientMap: toonGradient });
    ...
    const micMat = new THREE.MeshStandardMaterial({ color: 0x9e9e9e, metalness: 0.8, roughness: 0.2 });
    ...
    for (let i = 0; i < 2; i++) {
      ...
      const btn = new THREE.Mesh(btnGeo, suitLightMat);
      btn.position.set(0, 0.1 - i * 0.12, 0.32);
      bodyGroup.add(btn);
    }
    ...
    // Three eyes (triangle arrangement)
    this.leftPupil = null;
    this.rightPupil = null;
    const eyePositions = [
      { x: -0.12, y: 0.05, z: 0.22 },   // left
      { x: 0.12, y: 0.05, z: 0.22 },    // right
      { x: 0, y: 0.14, z: 0.2 },        // top (third eye)
    ];
    ...
    // Eyebrows (small ridges above eyes)
    ...
    // Camera body
    const camBodyGeo = new THREE.BoxGeometry(0.18, 0.12, 0.25);
    const camBody = new THREE.Mesh(camBodyGeo, camBodyMat);
    camBody.position.set(0, 0.08, 0.05);
    camGroup.add(camBody);

    // Camera lens barrel
    const camBarrelGeo = new THREE.CylinderGeometry(0.04, 0.05, 0.12, 16);
    const camBarrel = new THREE.Mesh(camBarrelGeo, camLensMat);
    camBarrel.rotation.x = Math.PI / 2;
    camBarrel.position.set(0, 0.08, 0.22);
    camGroup.add(camBarrel);

    // Camera lens glass
    const camGlassGeo = new THREE.CylinderGeometry(0.035, 0.035, 0.01, 16);
    const camGlass = new THREE.Mesh(camGlassGeo, camGlassMat);
    camGlass.rotation.x = Math.PI / 2;
    camGlass.position.set(0, 0.08, 0.28);
    camGroup.add(camGlass);

    // Camera viewfinder (eyepiece)
    const viewfinderGeo = new THREE.BoxGeometry(0.06, 0.04, 0.08);
    const viewfinder = new THREE.Mesh(viewfinderGeo, camBodyMat);
    viewfinder.position.set(0, 0.16, -0.05);
    camGroup.add(viewfinder);

    // Camera handle (top)
    const handleGeo = new THREE.BoxGeometry(0.04, 0.06, 0.12);
    const handle = new THREE.Mesh(handleGeo, camBodyMat);
    handle.position.set(0, 0.2, 0);
    camGroup.add(handle);

    // Recording light (red dot)
    const recLightGeo = new THREE.SphereGeometry(0.015, 8, 8);
    const recLight = new THREE.Mesh(recLightGeo, camRedMat);
    recLight.position.set(0.08, 0.14, 0.12);
    camGroup.add(recLight);
    this.recLight = recLight;

    // Camera light glow
    const camLight = new THREE.PointLight(0xff3333, 0.3, 1.5);
    camLight.position.set(0.08, 0.14, 0.12);
    ...
    // Initial pose — holding camera up, aiming forward
    this.rightArm.rotation.z = -0.3;
    this.rightArm.rotation.x = -0.5;
    this.rightElbow.rotation.x = -0.8;
