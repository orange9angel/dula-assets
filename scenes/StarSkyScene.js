...
export class StarSkyScene extends SceneBase {
  ...
  build() {
    ...
    // ---- Ground (far below, almost invisible) ----
    ...
    // ---- Distant city lights below (tiny points) ----
    const cityLightMat = new THREE.MeshBasicMaterial({ color: 0xffdd66, transparent: true, opacity: 0.4 });
    for (let i = 0; i < 40; i++) {
      const angle = (i / 40) * Math.PI * 2 + Math.random() * 0.3;
      const radius = 60 + Math.random() * 50;
      const light = new THREE.Mesh(new THREE.SphereGeometry(0.08, 4, 4), cityLightMat);
      light.position.set(Math.cos(angle) * radius, -12 + Math.random() * 2, Math.sin(angle) * radius);
