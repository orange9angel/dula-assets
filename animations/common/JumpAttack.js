...
export class JumpAttack extends AnimationBase {
  ...
  update(t, character) {
    ...
    // Phase 2: Leap (0.15-0.4) - jump up and forward
    else if (t < 0.4) {
      const p = (t - 0.15) / 0.25;
    ...
    // Phase 5: Recover (0.8-1.0) - return to fighting stance
      ...
      const ease = p * p;
      if (character.baseY !== undefined) {
        character.mesh.position.y = (character.baseY - 0.02) - ease * 0.04;
      }
      // Fighting stance: right back guard, left forward
      rArm.rotation.z = (rBaseZ - 0.3) - ease * 0.6;
      rArm.rotation.x = (rBaseX - 0.5) - ease * 0.2;
      if (lArm) {
        lArm.rotation.z = (lBaseZ + 0.2) + ease * 0.3;
        lArm.rotation.x = -0.2 - ease * 0.2;
      }
      if (rLeg) rLeg.rotation.x = 0.3 - ease * 0.05;
      if (lLeg) lLeg.rotation.x = 0.2 - ease * 0.0;
      character.mesh.rotation.x = 0.1 - ease * 0.02;
    }
  }
}
