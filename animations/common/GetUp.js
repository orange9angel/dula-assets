...
export class GetUp extends AnimationBase {
  ...
  update(t, character) {
    ...
    if (t < 0.2) {
      const p = t / 0.2;
      const ease = p * p;
      character.mesh.rotation.x = -1.7 + ease * 1.0;
      character.mesh.rotation.z = ease * 0.3;
    ...
    // Phase 2: Push up (0.2-0.5) - hands to ground, push up
      ...
      character.mesh.rotation.x = -0.7 + ease * 0.7;
    ...
    // Phase 4: Stand (0.7-1.0) - return to fighting stance
      ...
      const p = (t - 0.7) / 0.3;
      const ease = p * p;
      character.mesh.rotation.x = 0.05 + ease * 0.03;
      if (character.baseY !== undefined) {
        character.mesh.position.y = (character.baseY - 0.03) - ease * 0.03;
      }
      // Fighting stance: right back guard, left forward
      if (rArm) {
        rArm.rotation.z = (rBaseZ + 0.2) - ease * 1.1;
        rArm.rotation.x = -0.2 - ease * 0.5;
      }
      if (lArm) {
        lArm.rotation.z = (lBaseZ - 0.2) + ease * 0.7;
        lArm.rotation.x = -0.2 - ease * 0.2;
      }
      if (rLeg) rLeg.rotation.x = 0.4 - ease * 0.15;
      if (lLeg) lLeg.rotation.x = 0.3 - ease * 0.1;
    }
  }
}
